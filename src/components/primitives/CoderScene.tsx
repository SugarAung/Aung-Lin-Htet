...
function TypingArms({ reduced }: { reduced: boolean }) {
  ...
  return (
    <>
      ...
      <mesh ref={leftRef} position={[-0.82, -0.12, 0.28]}>
        ...
        <meshStandardMaterial color={NAVY} roughness={0.9} metalness={0.05} />
      </mesh>
      {/* Right forearm */}
...
function Scene({ reduced }: { reduced: boolean }) {
  return (
    <>
      ...
      <pointLight position={[-2, 2, 1]} intensity={0.2} color="#7c3aed" />
      ...
      <TiltGroup reduced={reduced}>
        ...
        >
          ...
          {/* ── Character — body (navy T-shirt) ── */}
          <Box size={[0.58, 0.75, 0.4]} position={[-1.1, 0.52, 0]} color={NAVY} />
          {/* Collar / shirt detail */}
          <Box size={[0.22, 0.18, 0.41]} position={[-1.1, 0.88, 0]} color={NAVY_LIGHT} />
          {/* Head */}
